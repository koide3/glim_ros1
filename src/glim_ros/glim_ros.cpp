...
GlimROS::GlimROS() {
  ...
  const std::string config_path =
    ros::package::getPath("glim") + config_ros.param<std::string>("glim_ros", "config_path", "/config");
  ...
  std::cout << console::bold_red << "You must carefully check and follow the licenses of ext modules" << console::reset
            << std::endl;
...
#else
    std::cerr << console::bold_red << "error: GPU frontend is selected although glim was built without GPU support!!"
